...
def main():
    ...
    # Check if the dataset dataroot is provided，也可以删，没啥大用处
    ...
    if args.text_usage == "inversion_adapter":
        ...
        vision_encoder = None
        processor = None
    #代码完成了模型的设备移动、训练步数的重新计算以及跟踪器的初始化和配置存储等任务。
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet), accelerator.accumulate(inversion_adapter):
                ...
                # Get the text embedding for conditioning删
                ...
                mask = torch.nn.functional.interpolate(mask, size=(512 // 8, 384 // 8))  #下采样8倍
                ...
                # Randomly mask out some of the inputs代码实现了随机屏蔽一部分输入的功能。这样可以模拟一些无条件的情况，对模型进行更全面的测试和评估。
