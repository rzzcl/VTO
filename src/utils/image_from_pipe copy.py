...
@torch.no_grad()
def generate_images_from_tryon_pipe(pipe: StableDiffusionTryOnePipeline,cascade_unet:Unet
                                    test_dataloader: torch.utils.data.DataLoader, output_dir: str, order: str,
                                    save_name: str, vision_encoder: CLIPVisionModelWithProjection,
    ...
    for idx, batch in enumerate(tqdm(test_dataloader)):
        ...
        mask_img = batch.get("inpaint_mask")   #???????为什么不是im_mask？
        ...
        #Generate cascade feature
        prompt_embeds = cascade_unet(cloth)
