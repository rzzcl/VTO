...
from cascade_unet.model_feature_unet import Unet
...
class StableDiffusionTryOnePipeline(DiffusionPipeline):
    ...
    def prepare_mask_latents(
        ...
        if return_intermediate:
            return masked_image_latents, masked_image_intermediate_features
        else:
            return 
            masked_image_latents
    ...
    def __call__(
            ...
            eta: float = 0.0,
            # prompt_embeds: Optional[torch.FloatTensor] = None,
            prompt_embeds: Optional[List[torch.FloatTensor]] = None,  #unet级联特征
            negative_prompt_embeds: Optional[torch.FloatTensor] = None,
        ...
        print(batch_size,"bs_tryon_pipe")
        ...
        # if self.emasc:
        #     masked_image_latents, intermediate_features = self.prepare_mask_latents(
        #         mask,
        #         masked_image,
        #         batch_size * num_images_per_prompt,
        #         height,
        ...
        # else:
        masked_image_latents = self.prepare_mask_latents(
            # mask,
            masked_image,
            batch_size * num_images_per_prompt,
            height,
            ...
            for i, t in enumerate(timesteps):
                ...
                if cloth_latents is not None:
                    latent_model_input = torch.cat(
                        [latent_model_input, masked_image_latents, pose_map.to(mask.dtype),
                        cloth_latents.to(mask.dtype)], dim=1)
                        print("cloth_latents is true!")
                else:
                    latent_model_input = torch.cat(
                        [latent_model_input,masked_image_latents, pose_map.to(mask.dtype)], dim=1)

                # predict the noise residual 改！
